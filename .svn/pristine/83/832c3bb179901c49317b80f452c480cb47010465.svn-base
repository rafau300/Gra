...
public class Server implements Serializable {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println(">>Wysylam potwierdzenia do klientow");
			...
			try {
				...
				for (i = 0; i < 10; i++) {
					if (d1.asteroidaHp[i] > 0) {
						...
						if (d1.asteroidaX[i]>=640) d1.asteroidaX[i]=-20;
						if (d1.asteroidaX[i]<-20) d1.asteroidaX[i]=640;
					}
					else if (d1.asteroidaHp[i]<=0 || d2.asteroidaHp[i]<=0){//gdy zestrzelono = odnowienie i delikatne przyspieszenie
						d1.asteroidaHp[i]=10;
						d1.asteroidaY[i]=-20;
						if(d1.asteroidaRY[i]<3) d1.asteroidaRY[i]+=1;
						/*if (d1.lvl[i]<4) d1.lvl[i]++;
						switch (d1.lvl[i]) {
						case 1: d1.asteroidaHp[i]=5;
						break;
						case 2: d1.asteroidaHp[i]=10;
						break;
						case 3: d1.asteroidaHp[i]=15;
						break;
						default: d1.asteroidaHp[i]=20;
						}*/
			...
			try {
				...
				for (i = 0; i < 10; i++) {
					if (d2.asteroidaHp[i] > 0) {
						...
						if (d2.asteroidaX[i]>=640) d2.asteroidaX[i]=-20;
						if (d2.asteroidaX[i]<=-20) d2.asteroidaX[i]=640;
					}
					else {
						d2.asteroidaHp[i]=10;
						d2.asteroidaY[i]=-20;
						if (d2.asteroidaRY[i]<3) d2.asteroidaRY[i]+=1;
						//if (d2.lvl[i]<4) d2.lvl[i]++;
						/*switch (d2.lvl[i]) {
						case 1: d2.asteroidaHp[i]=5;
						break;
						case 2: d2.asteroidaHp[i]=10;
						break;
						case 3: d2.asteroidaHp[i]=15;
						break;
						default: d2.asteroidaHp[i]=20;
						}*/
					}
				}
			} catch (Exception e) {
				System.err.println("2. Nie odebrano pakietu");
				//e.printStackTrace();
			}
